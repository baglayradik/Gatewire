...
- Модуль `GatewireTesting`: `StubNetwork` с заглушками по эндпоинтам, путям и условиям, очередями ответов,
  ошибками сети, задержками, перенаправлениями и записью запросов; изоляция параллельных тестов.
